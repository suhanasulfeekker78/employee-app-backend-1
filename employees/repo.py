...
async def create(db: AsyncSession, name: str, email: str, age:int|None, password_hash:str,address_data: dict | None) -> Employee:
...
async def search(db: AsyncSession, name: str | None) -> list[Employee]:
    ...
    stmt = select(Employee).where(Employee.deleted_at.is_(None)).options(
        selectinload(Employee.addresses), selectinload(Employee.departments)
    )
...
async def update_by_id(db: AsyncSession, id: int, updated_data: dict):
    ...
    stmt = update(Employee).where(Employee.id == id, Employee.deleted_at.is_(None)).values(**updated_data).returning(Employee)
...
async def add_department_link(db: AsyncSession, employee_id: int, department_id: int) -> None:
    dept_stmt = select(Department).where(Department.id == department_id, Department.deleted_at.is_(None))
    ...
    junction = Employee_X_Department(employee_id=employee_id, department_id=department_id)
    db.add(junction)
    await db.commit()


async def remove_department_link(db: AsyncSession, employee_id: int, department_id: int) -> None:
...
async def remove_address_link(db: AsyncSession, employee_id: int, address_id: int) -> None:
