...
from employees.repo import add_department_link,remove_department_link,remove_address_link
...
async def create_employee(db: AsyncSession, data: CreateEmployeeRequest) -> Employee:
    ...
    employee = await create(db, data.name, data.email, data.age, password_hash=hashed,address_data=address_dict)
...
async def update_employee(db: AsyncSession, id: int, data: UpdateEmployeeRequest) -> Employee:

    updated_fields = data.model_dump(exclude_none=True)

    if not updated_fields:
        raise BadRequestException("No modified tracking properties found inside request body")
...
async def attach_department_to_employee(db: AsyncSession, employee_id: int, department_id: int) -> None:
    await employee_by_id(db, employee_id) # Verify existence
    await add_department_link(db, employee_id, department_id)


async def detach_employee_from_department(db: AsyncSession, employee_id: int, department_id: int) -> None:
    await employee_by_id(db, employee_id) # Verify existence
    await remove_department_link(db, employee_id, department_id)


async def delete_employee_address(db: AsyncSession, employee_id: int, address_id: int) -> None:
    await employee_by_id(db, employee_id) # Verify existence
